...
# 前端
cd frontend && npm run dev         # 启动前端（端口 5173，API 代理到 localhost:3000）
...
| 代码 | 名称 | 明细表 |
|------|------|--------|
| SYS  | 压实度 | `biz_compaction_item`（部位、组数、材料、设计要求） |
...
- `printPdf`: 用 pizzip 直接编辑 docx 模板的 XML，填充数据后通过 Word COM（Windows）或 LibreOffice headless（Linux/Docker）转为 PDF

### PDF 生成

- **模板**: `temp/压实度（道路）（灌砂法）检测原始记录单.docx`（仅用于路基压实度）
- **方案**: 编辑 docx 内部 XML → 填充数据 → zip 重打包 → 转为 PDF
- **Windows**: 通过 PowerShell 调用 Word COM 自动化（`$doc.SaveAs([ref] $path, [ref] 17)`）
- **Linux/Docker**: 通过 `libreoffice --headless --convert-to pdf` 转换
- **环境变量**: 设置 `PDF_CONVERTER=libreoffice` 强制使用 LibreOffice
